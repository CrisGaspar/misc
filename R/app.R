...
format_columns <- function(data_table) {
  data_table %>% 
    formatCurrency(intersect(COLUMNS_COUNTER, colnames(data_table)), currency = FORMAT_SETTINGS_COUNTER$SYMBOL, mark = FORMAT_SETTINGS_COUNTER$SEPARATOR, digits = FORMAT_SETTINGS_COUNTER$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_CURRENCY_0_DECIMALS, colnames(data_table)), currency = FORMAT_SETTINGS_CURRENCY_DEFAULT$SYMBOL, mark = FORMAT_SETTINGS_CURRENCY_DEFAULT$SEPARATOR, 
                     digits = FORMAT_SETTINGS_CURRENCY_DEFAULT$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_CURRENCY_2_DECIMALS, colnames(data_table)), currency = FORMAT_SETTINGS_CURRENCY_2_DECIMALS$SYMBOL, mark = FORMAT_SETTINGS_CURRENCY_2_DECIMALS$SEPARATOR, 
                     digits = FORMAT_SETTINGS_CURRENCY_2_DECIMALS$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_PERCENT_1_DECIMAL, colnames(data_table)), currency = FORMAT_SETTINGS_PERCENT_1_DECIMAL$SYMBOL, mark = FORMAT_SETTINGS_PERCENT_1_DECIMAL$SEPARATOR, 
                     digits = FORMAT_SETTINGS_PERCENT_1_DECIMAL$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_PERCENT_2_DECIMALS, colnames(data_table)), currency = FORMAT_SETTINGS_PERCENT_2_DECIMALS$SYMBOL, mark = FORMAT_SETTINGS_PERCENT_2_DECIMALS$SEPARATOR, 
                     digits = FORMAT_SETTINGS_PERCENT_2_DECIMALS$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_PERCENT_4_DECIMALS, colnames(data_table)), currency = FORMAT_SETTINGS_PERCENT_4_DECIMALS$SYMBOL, mark = FORMAT_SETTINGS_PERCENT_4_DECIMALS$SEPARATOR, 
                     digits = FORMAT_SETTINGS_PERCENT_4_DECIMALS$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_PERCENT_5_DECIMALS, colnames(data_table)), currency = FORMAT_SETTINGS_PERCENT_5_DECIMALS$SYMBOL, mark = FORMAT_SETTINGS_PERCENT_5_DECIMALS$SEPARATOR, 
                     digits = FORMAT_SETTINGS_PERCENT_5_DECIMALS$DECIMALS) %>% 
    formatCurrency(intersect(COLUMNS_PERCENT_6_DECIMALS, colnames(data_table)), currency = FORMAT_SETTINGS_PERCENT_6_DECIMALS$SYMBOL, mark = FORMAT_SETTINGS_PERCENT_6_DECIMALS$SEPARATOR, 
...
renderDT_formatted <- function(data_frame, no_table_header = F) {
  if (no_table_header) {
    ...
    renderDT(datatable(data_frame, options = list(dom = 't', bSort = FALSE), colnames = NULL) %>% format_columns, selection = 'none', server = F)
  }
  else {
    # Do not show search box
    # Before rendering format the numbers to display in currency format
    renderDT(datatable(data_frame, options = list(searching = FALSE)) %>% format_columns, selection = 'none', server = F)
  }
}
